...
concesiones_biomasa <- readRDS(
  "data/concesiones_biomasa.rds"
)

concesiones_poli_biomasa <- readRDS(
  "data/concesiones_poli_biomasa.rds"
)

AAA_biomasa <- readRDS(
  "data/AAA_biomasa.rds"
)

ECMPO_biomasa <- readRDS(
  "data/ECMPO_biomasa.rds"
)

Macrozonas_biomasa <- readRDS(
  "data/Macrozonas_biomasa.rds"
)

ASP_biomasa <- readRDS(
  "data/ASP_biomasa.rds"
)
...
colorgremio <- colorFactor(
  palette = colores_gremio,
  domain = names(colores_gremio),
  na.color = "#BDBDBD"
)
...
server <- function(input, output, session) {
  ...
  output$mapa_biomasas <- renderLeaflet({
    ...
    leaflet(
      ...
      addMapPane("pane_AAA", zIndex = 400) %>%
      addMapPane("pane_Macrozonas", zIndex = 403) %>%
      addMapPane("pane_ECMPO", zIndex = 405) %>%
      addMapPane("pane_ASP", zIndex = 408) %>%
      addMapPane("pane_comunas", zIndex = 410) %>%
      addMapPane("pane_acs", zIndex = 420) %>%
      addMapPane("pane_centros_poligonos", zIndex = 430) %>%
      addMapPane("pane_centros_puntos", zIndex = 440) %>%
      ...
      addCircleMarkers(
        data = concesiones_biomasa,
        lng = ~Longitud,
        lat = ~Latitud,
        
        radius = 4,
        
        color = ~colorgremio(Gremio),
        fillColor = ~colorgremio(Gremio),
        
        weight = 1,
        opacity = 0.9,
        fillOpacity = 0.8,
        options = pathOptions(
          pane = "pane_centros_puntos"
        ),
        
        popup = ~paste0(
          "<strong>Codigo centro:</strong> ",
          ifelse(
            is.na(`Codigo centro`),
            "Sin informacion",
            `Codigo centro`
        ...
        group = "Centros de cultivo puntos"
      ) %>%
      
      addPolygons(
        data = concesiones_poli_biomasa,
        ...
        group = "Centros de cultivo polígonos"
      ) %>%
      ...
      addPolygons(
        ...
        fillColor = "blue",
        fillOpacity = 0.4,
        label = ~paste("ECMPO ",enc2utf8(Name)),
        labelOptions = labelOptions(
          style = list("font-size" = "13px")
        ),
        group = "ECMPO")%>%
      addPolygons(
        data = Macrozonas_biomasa,
        color = "blue",       
        weight = 1.2,
        options = pathOptions(
          pane = "pane_Macrozonas"
        ),
        fillColor = "lightblue", 
        fillOpacity = 0,
        label = ~paste("Macrozona ",enc2utf8(REP_SUBP_1)),
        labelOptions = labelOptions(
      ...
      addPolygons(
        ...
        fillColor = "green", 
        fillOpacity = 0,      
        label = ~paste(enc2utf8(`designacio`)," ",
                       enc2utf8(`Nombre sin tilde`)),
        labelOptions = labelOptions(
          style = list("font-size" = "15px")  # Cambia el tamaño del texto
        ),
        group = "ASP"
      )%>%
      ...
      addLayersControl(
        overlayGroups = c(
          "Comunas",
          "ACS",
          "Centros de cultivo puntos",
          "Centros de cultivo polígonos",
          "AAA",
          "ECMPO",
          "Macrozonas",
          "ASP"
      ...
      hideGroup("Centros de cultivo puntos")%>%
      hideGroup("Centros de cultivo polígonos")%>%
      hideGroup("AAA")%>%
      hideGroup("ECMPO")%>%
      hideGroup("Macrozonas")%>%
      hideGroup("ASP")%>%
      addScaleBar(
      ...
      addLegend(
        position = "bottomright",
        colors = unname(colores_gremio),
        labels = names(colores_gremio),
        opacity = 1,
        title = HTML(
          "<strong>Gremio centros de cultivo</strong>"
        )
      ) %>%
